...
- **`suspected_malfunctioning`** (1 station): reporting values that cannot be reconciled with any neighbouring sensor, or with the device's own second channel. Currently Klungkung by Lumi Clinic (PurpleAir): one of its two laser channels stopped on 26 August 2026 (channel A reads 0.1 µg/m³ while B reads 35), and its published figure, the combination of the two, fell to about a quarter of its prior level against neighbouring stations. Readings through 25 August are sound; later ones are far too low. It stays on the map, grey and labelled, and in the archive, flagged on every row. The only earlier entry, an IQAir pin labelled "Kopernik (Mas, Ubud)", was withdrawn in September 2026: not a faulty sensor but not a sensor at all (§8.5).
